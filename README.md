...
**KRASMTPred** is a multi-task deep neural network (DNN)-based predictive framework developed for the identification of **GTPase Kirsten rat sarcoma virus oncogene homologue (GTPase KRas) inhibitors**.
...
├── krasmtpred_predict.py              # Prediction pipeline script
├── padel_scaler.pkl                   # Pre-trained normalization model
├── krasmt_model.h5                    # Trained multi-task DNN model
...
```bash
docker run --rm -v "${PWD}:/WorkPlace" ghcr.io/pradnyajk/krasmtpred:latest sample.csv output.csv
```
...
```bash
git clone https://github.com/PGlab-NIPER/KRASMTPred.git
cd KRASMTPred
git lfs pull
```
...
```bash
docker run --rm -v "${PWD}:/WorkPlace" krasmtpred sample.csv output.csv
```
...
If you use **KRASMTPred** in your research, please cite:
